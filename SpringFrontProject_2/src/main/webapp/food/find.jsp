			...
			<div class="text-center">
				<ul class="pagination">
					<c:if test="${startpage>1}">
						<li><a href="find.do?address=${address}&page=${startpage-1}">&lt;</a></li>
					</c:if>
					<c:forEach var="i" begin="${startpage}" end="${endpage}">
						<li ${i==curpage?"class=active":""}><a href="find.do?address=${address}&page=${i}">${i}</a></li>	
					</c:forEach>
					<c:if test="${endpage<totalpage}">
						<li><a href="find.do?address=${address}&page=${endpage+1}">&gt;</a></li>
					</c:if>
				</ul>
			</div>
		</div>
