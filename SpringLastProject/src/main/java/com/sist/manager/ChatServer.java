...
public class ChatServer {
	...
	// 접속 해제
	// Session session = 접속자에 대한 구분자, 접속자마다 번호 부여
	@OnClose
	public void onClose(Session session) throws Exception{
		// 저장된 데이터 전체를 모아서 처리, Collection은 전체 Iterator를 가지고 있다
		Iterator<Session> it=users.keySet().iterator();
		while(it.hasNext()) {
			Session ss=it.next(); // 저장되어있는 모든 사용자를 한명씩 읽어 온다
